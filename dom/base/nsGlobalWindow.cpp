...
nsGlobalWindow::FreeInnerObjects()
{
  ...
  mozilla::dom::workers::CancelWorkersForWindow(this->AsInner());
                               ...
                               bool aForceReuseInnerWindow)
{
  ...
  if (IsInnerWindow()) {
    ...
    if (mOuterWindow->GetCurrentInnerWindow() != this->AsInner()) {
...
nsGlobalWindow::SetDocShell(nsIDocShell* aDocShell)
{
  ...
  if (!mChromeEventHandler) {
    ...
    if (parentWindow.get() != this->AsOuter()) {
      mChromeEventHandler = parentWindow->GetChromeEventHandler();
    }
    else {
      mChromeEventHandler = NS_NewWindowRoot(this->AsOuter());
    }
  }
...
#ifdef DEBUG
  MOZ_ASSERT(!aOpener || aOpener->IsOuterWindow());
#endif
...
FullscreenTransitionTask::Run()
{
  ...
  if (stage == eBeforeToggle) {
    ...
    if (!mWindow->SetWidgetFullscreen(eForFullscreenAPI, mFullscreen, mWidget,
                                      mScreen)) {
                                    ...
                                    nsIWidget* aWidget, nsIScreen* aScreen)
{
  ...
  nsresult rv = aReason == eForFullscreenMode ?
...
nsGlobalWindow::FinishFullscreenChange(bool aIsFullscreen)
{
  ...
  if (!mWakeLock && mFullScreen) {
    ...
    mWakeLock = pmService->NewWakeLock(NS_LITERAL_STRING("DOM_Fullscreen"),
                                       this->AsOuter()->GetCurrentInnerWindow(), rv);
  ...
  NS_IMETHOD Run()
  {
    ...
    if (!skipNukeCrossCompartment && window) {
      ...
      nsGlobalWindow* currentInner =
        inner ? nsGlobalWindow::Cast(nsPIDOMWindowInner::From(inner)) :
        nsGlobalWindow::Cast(nsPIDOMWindowOuter::From(outer)->GetCurrentInnerWindow());
      ...
      if (obj && !js::IsSystemCompartment(js::GetObjectCompartment(obj))) {
        js::NukeCrossCompartmentWrappers(cx,
                                         ...
                                         inner ? js::DontNukeWindowReferences : js::NukeWindowReferences);
      }
    }
...
nsGlobalWindow::GetOwnerGlobalForBindings()
{
  ...
  return nsPIDOMWindowOuter::GetFromCurrentInner(this->AsInner());
}
                                ...
                                bool aFreezeWorkers)
{
  ...
  if (!suspended) {
    ...
    if (aFreezeWorkers) {
      mozilla::dom::workers::FreezeWorkersForWindow(this->AsInner());
    } else {
...
nsGlobalWindow::ResumeTimeouts(bool aThawChildren, bool aThawWorkers)
{
  ...
  if (shouldResume) {
    ...
    if (aThawWorkers) {
      mozilla::dom::workers::ThawWorkersForWindow(this->AsInner());
    } else {
