...
AskWindowToExitFullscreen(nsIDocument* aDoc)
{
  if (XRE_GetProcessType() == GeckoProcessType_Content) {
    ...
    if (nsPIDOMWindowOuter* win = aDoc->GetWindow()) {
      win->SetFullscreenInternal(eForFullscreenAPI, false);
    }
  }
}
...
public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    if (nsPIDOMWindowOuter* win = mDocuments[0]->GetWindow()) {
      win->SetFullscreenInternal(eForForceExitFullscreen, false);
    }
    return NS_OK;
  }
...
nsDocument::RequestFullScreen(UniquePtr<FullscreenRequest>&& aRequest)
{
  ...
  if (XRE_GetProcessType() == GeckoProcessType_Content) {
    ...
    rootWin->SetFullscreenInternal(eForFullscreenAPI, true,
